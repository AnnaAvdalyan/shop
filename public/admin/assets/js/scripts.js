...
function updateThumbnail(dropZoneElement, file) {
    ...
    $(document).ready(function () {
        ...
        // formData.append('image', inputElement.files);
        $.ajax({
            url: 'galleries',
            data: formData,
            type: "POST",
            ...
            success: function (res) {
                ...
                let html = `<div class="col-3">
                                    <img src="${res.img}" alt="">
                                    <form action="http://127.0.0.1:8000/galleries/${res.id}" method="post" class="deleteGallery">
                                        <input type="hidden" name="_method" value="DELETE">
                                         <button type="submit" class="btn btn-primary"><em class="icon ni ni-trash"></em><span>Удалить </span></button>
                                </form>
                                </div>`;
                $('.allGellery').prepend(html)
            },
            error: function (error) {
                console.log(error)
            }
...
$(document).ready(function () {
    ...
    $(document).on('submit', '.deleteGallery', function (e) {
        e.preventDefault();
        let $this = $(this);
    ...
    $('.single__gallery img').on('click', function (e) {
        e.preventDefault();
        $(this).toggleClass('active')
            let array = [];
            for (let i = 0; i < $('.single__gallery img.active').length; i++) {
                array.push($('.single__gallery img.active').eq(i).attr('data-id'));
            }
            console.log(array);
        $('.single__gallery form input[name="productAddCategory"]').val(array)
    })
    $('.single__gallery form').on('submit', function (e) {
        e.preventDefault();
        let $this = $(this);
        let data = $($this).serialize();
        let url = $($this).attr('action');
